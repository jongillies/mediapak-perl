#!/usr/bin/perl -w
my $PROGRAM_NAME="dbcheck";
my $PROGRAM_DESC="Check Database vs File System Media";
...
#
for my $sha1 ( keys %localFile )
{
	my ( $filename, $path) = split (/\t/, $localFile{$sha1});

print ("ZZZZZZZZZZZZZZZZ\tfilename=>$filename<\tpath=>$path<\n\n\n");
...
for my $sha1 ( keys %database )
{
	...
	{	
		...
		if ( $localFile{$sha1} )
		{
			print ("FOUND_LOCAL\t$sha1\t$filename\t$localFile{$sha1}\n");
		}
		else
		{
			print ("NOT_FOUND_LOCAL\t$sha1\t$filename\t$filename\n");
			$totalErrors++;
		}
	}
}

...
# Sister program: create-link-data.pl
...
#
my $PROGRAM_NAME="filelink";
my $PROGRAM_DESC="Create Symbolic Links from Metadata";
...
sub usage
{
	...
	print ("Usage: $PROGRAM_NAME [-d] [-f link-file] [-p path to link] [-i include-ext] [-c offline-cache-folder] [path] [path ...]\n" );
	...
	print ("        [-p] path to link to\n");
	print ("        [-f] Link data file\n");
...
my $linkFile;                           # -f link file
my $linkPath;                           # -p link file

# Parse and validate command line options
if (! getopts('di:c:f:p:', \%cmdline_option) )
{
...
$linkFile = $cmdline_option{f};
$linkPath = $cmdline_option{p};

if ( !defined($linkPath) )
{
	print (STDOUT "ABORT\tPlease specify a -p option!\n");
	usage ();
}

if ( !defined($linkFile) )
{
	print (STDOUT "ABORT\tPlease specify a -f option!\n");
	usage ();
}
...
require ("lib-do_symlink.pl");

# Read Link File
my @links = read_file ($linkFile);
foreach my $link (@links)
{
	(my $sha1, my $path, my $fileName) = split (/\t/, $link);

	if ( defined( $metaData{$sha1} ) )
	{
		my $source = File::Spec->canonpath( $metaData{$sha1}{path} );
		my $dest = File::Spec->canonpath( File::Spec->catfile ($linkPath, $path, $fileName) );
